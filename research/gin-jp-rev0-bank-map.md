# Gin Japanese revision 0 bank map

The Japanese origin candidate `gin-jp-rev0` is 1 MiB and divides exactly into 64 physical 16 KiB ROM banks. `analysis/gin-jp-rev0-bank-fingerprints.json` records a SHA-256 fingerprint, Shannon entropy, distinct-byte count, and zero/`0xff` counts for every bank. `analysis/banks.csv` maps bank 0 to the fixed `0x0000–0x3fff` CPU window and switchable banks to `0x4000–0x7fff`.
